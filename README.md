...
The DCoSL protocol is based on the premise that the decentralized curation of a simple list is the <i>atomic unit</i> and the <i>defining feature</i> of the decentralized web. DCoSL strives to turn a difficult and often seemingly overwhelming challenge -- a web of trust that truly gains traction -- into a tractable one by breaking it down into bite size steps: one simple list at a time. Just about anything can be expressed in terms of simple lists: a list of trusted lightning nodes, a list of nostr relays, a list of user properties (display_name, etc), and so on. Successive compounding of simple lists can result in complex lists (e.g. to segregate list items into subsets, with subsets being another simple list). Out of simple lists, data structures can be constructed that are as complex as they need to be.
