...
The DCoSL protocol is based on the premise that the decentralized curation of a simple list is the <i>atomic unit</i> and the <i>defining feature</i> of the decentralized web. DCoSL strives to turn a difficult and often seemingly overwhelming challenge -- a web of trust that truly gains traction -- into a tractable one by breaking it down into bite size steps: one simple list at a time. Just about anything can be expressed in terms of simple lists: a list of trusted lightning nodes, a list of nostr relays, a list of user properties (display_name, etc). With DCoSL, developers provide tools to allow users to farm out various decisions to communities of users. By choosing a list or lists carefully, a magical thing will often emerge called loose consensus: Alice's WoT and Bob's WoT, though they are not identical, are very likely to have significant overlap, and will likely farm out the curation of said list to the same small group of trusted list Curators. For any given list, although there is no guarantee, there is a good chance that Alice and Bob will end up using the exact same (or at least almost the same) list! 
